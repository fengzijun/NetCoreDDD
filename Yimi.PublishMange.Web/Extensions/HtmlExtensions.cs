...
namespace Yimi.PublishManage.Web.Extensions
{
    public static class HtmlExtensions
    {
            ...
            if (model.ShowPagerItems && (model.TotalPages > 1))
            {
                if (model.ShowFirst)
                {
                    ...
                    if ((model.PageIndex >= 3) && (model.TotalPages > model.IndividualPagesDisplayedCount))
                    {
                        ...
                        {
                            links.Append(html.RouteLink(model.FirstButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty/*localizationService.GetResource("Pager.FirstPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        else
                        {
                            links.Append(html.ActionLink(model.FirstButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty/*localizationService.GetResource("Pager.FirstPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        links.Append("</li>");
                    }
                }
                if (model.ShowPrevious)
                {
                    ...
                    if (model.PageIndex > 0)
                    {
                        ...
                        if (model.UseRouteLinks)
                        {
                            links.Append(html.RouteLink(model.PreviousButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty /*localizationService.GetResource("Pager.PreviousPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        else
                        {
                            links.Append(html.ActionLink(model.PreviousButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty /*localizationService.GetResource("Pager.PreviousPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        links.Append("</li>");
                    }
                }
                if (model.ShowIndividualPages)
                {
                    ...
                    for (int i = firstIndividualPageIndex; i <= lastIndividualPageIndex; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if (model.UseRouteLinks)
                            {
                                links.Append(html.RouteLink((i + 1).ToString(), model.RouteActionName, model.RouteValues, new { title = string.Empty /* String.Format(localizationService.GetResource("Pager.PageLinkTitle"), (i + 1))*/, @class = "page-link" }).ToHtmlString());
                            }
                            else
                            {
                                links.Append(html.ActionLink((i + 1).ToString(), model.RouteActionName, model.RouteValues, new { title = string.Empty /* String.Format(localizationService.GetResource("Pager.PageLinkTitle"), (i + 1))*/, @class = "page-link" }).ToHtmlString());
                            }
                            links.Append("</li>");
                        }
                    }
                ...
                if (model.ShowNext)
                {
                    ...
                    if ((model.PageIndex + 1) < model.TotalPages)
                    {
                        ...
                        if (model.UseRouteLinks)
                        {
                            links.Append(html.RouteLink(model.NextButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty /* localizationService.GetResource("Pager.NextPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        else
                        {
                            links.Append(html.ActionLink(model.NextButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty /* localizationService.GetResource("Pager.NextPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        links.Append("</li>");
                    }
                }
                if (model.ShowLast)
                {
                    ...
                    if (((model.PageIndex + 3) < model.TotalPages) && (model.TotalPages > model.IndividualPagesDisplayedCount))
                    {
                        ...
                        if (model.UseRouteLinks)
                        {
                            links.Append(html.RouteLink(model.LastButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty /* localizationService.GetResource("Pager.LastPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        else
                        {
                            links.Append(html.ActionLink(model.LastButtonText, model.RouteActionName, model.RouteValues, new { title = string.Empty /*localizationService.GetResource("Pager.LastPageTitle")*/, @class = "page-link" }).ToHtmlString());
                        }
                        links.Append("</li>");
                    }
                }
        ...
        //            {
        //                links.Append(html.RouteLink(x.ToString(), "TopicSlugPaged", new { id = forumTopicId, pageNumber = (x), slug = forumTopicSlug }, new { title = string.Empty /*String.Format(localizationService.GetResource("Pager.PageLinkTitle"), x.ToString())*/ }).ToHtmlString());
        //                if (x < totalPages)
        //                {
        //                    links.Append(", ");
        //                }
        ...
        //        else
        //        {
        //            links.Append(html.RouteLink("1", "TopicSlugPaged", new { id = forumTopicId, pageNumber = (1), slug = forumTopicSlug }, new { title = string.Empty /* String.Format(localizationService.GetResource("Pager.PageLinkTitle"), 1) */ }).ToHtmlString());
        //            links.Append(" ... ");

        //            for (int x = (totalPages - 2); x <= totalPages; x++)
        //            {
        //                links.Append(html.RouteLink(x.ToString(), "TopicSlugPaged", new { id = forumTopicId, pageNumber = (x), slug = forumTopicSlug }, new { title = string.Empty /* String.Format(localizationService.GetResource("Pager.PageLinkTitle"), x.ToString()) */ }));
