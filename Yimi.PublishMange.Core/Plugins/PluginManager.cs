...
using Yimi.PublishManage.Core.ComponentModel;
using Yimi.PublishManage.Core.Configuration;
using System.Runtime.Loader;
using Yimi.PublishMange.Core;

namespace Yimi.PublishManage.Core.Plugins
{
    /// <summary>
    ...
    public class PluginManager
    {
        ...
        /// Initialize
        /// </summary>
        public static void Initialize(ApplicationPartManager applicationPartManager, YimiConfig config)
        {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(YimiVersion.CurrentVersion, StringComparer.OrdinalIgnoreCase))
                        {
        ...
        private static Assembly PerformFileDeploy(FileInfo plug, ApplicationPartManager applicationPartManager)
        {
            if (plug.Directory == null || plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name + " file exists in a folder outside of the allowed Yiminode folder hierarchy");
            ...
            {
                throw new InvalidOperationException($"The plugin directory for the {plug.Name} file exists in a folder outside of the allowed Yiminode folder hierarchy - exception because of {shadowCopiedPlug.FullName} - exception: {ex.Message}");
            }
        }
